...
// NewREST returns a RESTStorage object that will work against Ports. The Port
// name encodes the VPC VNI and the IP, so creating a Port is an atomic IP claim
// — etcd name uniqueness serializes concurrent allocators (AlreadyExists on
// collision), no server-side allocator required.
