// cozyplane-kpr is a SEPARATE module on purpose: importing Cilium's
// load-balancer control plane drags in ~394 modules and a ~122 MB binary, which
// must not contaminate the main cozyplane module's go.mod or fatten the agent
// image (docs/kube-proxy-replacement.md, "Architecture"). The main module never
// imports Cilium; only this one does. Pin v1.19.x and upgrade deliberately —
// Cilium makes no API-stability promise for pkg/.
module github.com/lllamnyp/cozyplane/kpr
