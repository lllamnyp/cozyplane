...
- [ ] **LoadBalancer ingress** (design needed): deliver inbound traffic addressed to a Service's LB IP — the public path for VPC-backed services (fabric-IP endpoints, bridge translates; see services-in-vpc.md § Composition). Requirements: no NodePort in the path (MetalLB precedent — LB IPs work without it), and **`externalTrafficPolicy: Local` preserving the client source** (deliver only to node-local ready backends). Plausible shape: announce + DNAT at `from_uplink`, reusing the floating-IP advertisement/delivery machinery at net 0 — [#13](../../issues/13)
