...
- [ ] **Two API groups, split by concern: the fabric plane and the tenant plane** — **design: [api-groups.md](api-groups.md)** (awaiting review). Forced by a real bug: a CRD keeps publishing OpenAPI paths after an APIService takes its group over, the specs collide (`duplicated path .../vpcs/{name}`), the group's schema never serves, and `kubectl apply` of every cozyplane object fails client-side with "failed to download openapi" while core types keep working — latent since the chart split, found 2026-07-12. The fix is structural rather than policed: `fabric.cozystack.io` (CRDs, ships with the CNI — fabric IPAM and node state, dependency floor = the kube API and nothing else) and `sdn.cozystack.io` (aggregated only, never CRDs — VPCs, Ports, policies, everything tenant-facing). Disjoint kinds ⇒ disjoint paths ⇒ the collision cannot occur; the takeover machinery is deleted, not fixed; and the default network stops depending on cozyplane's own apiserver — which it never really did. Interim for pre-split clusters: `--remove-bootstrap-crds` (default on) cleans the old single-group CRDs
