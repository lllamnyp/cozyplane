...
- [ ] Floating Services: a public address fronting a `ServiceVIP` (LoadBalancer-for-tenants; controller-elected anchor on a backend node, local-only selection, source-preserving) — **design draft: [floating-services.md](floating-services.md)** (awaiting review)
