...
- [ ] **Node-originated egress to a *remote* pod** — the datapath steers node-originated traffic (kubelet probes) to `to_pod` via a `/32 fabricIP → veth` route, which only exists for a **local** pod; a host-sourced packet to a pod on another node is never encapsulated into the overlay, so **cross-node host→pod times out** (same-node host→pod and pod→pod east-west both work). Surfaced running the full Cozystack platform: the hostNetwork kube-apiserver can't reach admission-webhook pods on other nodes, so `cert-manager` (and ~60 HRs behind it — kubevirt, cluster-api, kamaji, linstor) fail to converge. Fix: a node-origin → remote-pod path that enters the Geneve overlay for the host netns as source (mirroring `from_pod`/`from_overlay`). Diagnosis + measurements in [bringup-field-notes.md](bringup-field-notes.md#5-admission-webhooks-fail-cross-node--node-origin--remote-pod-open)
...
- [ ] Import Cilium's LB control plane + socket LB (`pkg/loadbalancer`, `pkg/socketlb`, pre-compiled `bpf_sock.o`) as a separate `cozyplane-kpr` component — **design: [kube-proxy-replacement.md](kube-proxy-replacement.md)**; **increments 1–2 done (`kpr/`, `deploy/kpr-daemonset.yaml`, `test/kpr-e2e.sh`): lbcell reconciles Services→pinned LB maps + committed `bpf_sock.o` attached at the cgroup root. Proven on a `kubeProxyMode: none` kind cluster (no service proxy at all) — from a pod, TCP + UDP ClusterIP load-balance and cluster DNS resolves; the UDP path exercises the reverse `recvmsg` translation. Packaged as an image + DaemonSet**. Forcing function: retaining Cilium only for KPR/host-policy keeps a *second IPAM authority* on the podCIDR — it collided a system pod's IP with Cilium's `reserved:health` (cross-node-unreachable; [#12](../../issues/12)) and its unmanaged-pod-watcher churns cozyplane pods. Dropping Cilium (KPR here + host-firewall/NetworkPolicy follow-ups) removes that whole class.
- [ ] Per-packet service fallback in cozyplane's hooks (external NodePort, VM-guest ClusterIP) — needed before kube-proxy can be removed. **Designed** ([kube-proxy-replacement.md](kube-proxy-replacement.md) increment-3 design pass): reuse the existing `svc_forward`/`svc_return` ClusterIP DNAT — the gap is that they're gated to VPC pods (`srcnet && !is_gw`), so net-0 VM-guest traffic falls to the kernel; un-gate for net 0 + add a `from_uplink` NodePort DNAT, and feed `svc_vips` at net 0 from cozyplane-kpr's StateDB tables (ownership partitioned by net). Implementation is the next chunk (touches the production datapath; tested on the cozyplane-CNI harness).
