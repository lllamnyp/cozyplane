...
- [ ] **Local-VIP mode: node-owned external addresses** — [lb-ingress.md](lb-ingress.md) § "Open gap". An external address that is one of the node's *own* addresses is accepted by kpr but not delivered: the agent resolves the interface a VIP routes through to build the `bpf_redirect_neigh` reply path, a node-owned address routes via `lo`, which has no MAC, and `EnsureFloatingUplink` refuses it — so the packet falls through to the host stack and gets an RST. This is the normal shape on clouds that NAT a public address onto the instance's primary private address (OCI, GCP, AWS), i.e. not exotic. Needs new `from_uplink` behaviour — intercept on the interface the packet arrived on and reply through normal routing rather than requiring a floating uplink MAC — so it wants its own design pass. Until then such clusters must publish the host ingress through a floating **secondary** address or a host-network proxy
