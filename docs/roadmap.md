...
- [ ] **Two API groups, one CRD-served and one aggregated** — **design: [api-groups.md](api-groups.md)** (awaiting review). A CRD keeps publishing OpenAPI paths after an APIService takes its group over, so the two specs collide (`duplicated path .../vpcs/{name}`), the group's schema never serves, and `kubectl apply` of every cozyplane object fails client-side with "failed to download openapi" — latent since the chart split, found 2026-07-12. Policing it (whoever installs second deletes the other's objects) races Helm and needs the apiserver to hold cluster-wide CRD-delete; splitting the group by owner makes the collision impossible instead. Interim: the apiserver removes the bootstrap CRDs at takeover (`--remove-bootstrap-crds`, default on) — which becomes the migration tool for pre-split clusters
