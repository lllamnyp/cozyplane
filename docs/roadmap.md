...
2. **Floating-IP HA beyond L2 — BGP** ([floating-ha.md](floating-ha.md) § increment
   2). Increments 0 and 1 are **done**: attraction is now separate from delivery
   and elected, so a migration makes no L2 claim at all. What remains is L3
   attraction — several nodes advertising the same /32, the fabric ECMPing across
   them, BFD withdrawing a dead one, and no shared L2 required.
   `ExternalPool.spec.advertisement` already reserves the `L2 | BGP` enum for it
   (presently dead — nothing reads it); the speaker belongs outside the datapath (§3).
