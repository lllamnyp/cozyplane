...
- [x] **`VPCGateway` — the VPC's declared north-south boundary** ([north-south.md](north-south.md) increment 1). A kind, not a field: `VPC.spec.egress.natGateway` was a bool on an object the tenant owns, so **a tenant granted itself internet**. Creating a gateway now needs the **`attach` verb on the referenced `ExternalPool`** (the `export`/`peer` escalation-gate pattern), so the operator grants the pool and the tenant opens its own door onto it. A VPC has exactly one boundary (oldest wins; `EffectiveGateway` lives in the API package because the controller, the CNI and the agent must agree on it without coordinating). **Tenet 7 is enforced:** `vpc_ingress[net]` gates `lb_ingress`, so a `Service type=LB` can no longer open a door into a tenant's VPC just by naming its pod as a backend — refusals counted in `ns_denied[door]`, kept out of the byte meter because a refused packet did not cross
