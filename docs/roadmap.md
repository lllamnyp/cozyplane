...
- [ ] Network policy / security groups within a VPC — **v1 + peered-group refs done** ([security-groups.md](security-groups.md)): east-west group-to-group ingress, destination-side eBPF (`sg_members`/`sg_rules`, TCP SYN-gate, per-VPC id allocation, membership from stamped pod labels); **peered-VPC group refs** (`from: {group, vpc}`) made authoritative by a Geneve identity TLV (source net from the veth, immune to cross-tenant net spoofing) — all dev4-validated. Outstanding (v2): egress, north-south `from.cidr`, FQDN, label-change-follows membership, `from_pod` source-IP RPF
