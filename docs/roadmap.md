...
- [ ] **Host firewall egress** — gate node-*originated* traffic (node→pod, node→internet); the natural home for making node→pod gating expressible (upstream NetworkPolicy has no "kubelet" peer vocabulary). Needs its own design pass: the failure mode is the agent severing its own apiserver access, so the K8s-contract minimum stays an always-allow — everything beyond it becomes explicit operator policy. Flow-ownership rationale in [policy-layers.md](policy-layers.md)
