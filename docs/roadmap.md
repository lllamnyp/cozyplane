...
- [~] **LoadBalancer ingress** — **design + increment 1 built: [lb-ingress.md](lb-ingress.md)**: *delivery only* — cozyplane consumes `status.loadBalancer.ingress` (whoever wrote it: CCM, MetalLB, a human), honours `ipMode`, and DNATs LB-IP traffic at `from_uplink` to node-local ready backends with the client source preserved (`externalTrafficPolicy: Local`; no NodePort in the path); allocation/announcement/provisioning are the LB implementation's job, out of scope. **Increment 1 (v4, default-net backends) e2e-covered** — external client → LB IP served source-preserving, backend-less node correctly refuses — **and dev-cluster-validated as the full composition** (MetalLB allocates + L2-advertises, cozyplane delivers). Left: v6 + VPC-pod backends (increment 2), `loadBalancerSourceRanges` (fast-follow) — [#13](../../issues/13)
