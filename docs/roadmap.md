...
2. **Floating-IP HA** — **[floating-ha.md](floating-ha.md)**, in progress. Today
   attraction and delivery are the same decision, so a migrating VM's public
   address is re-pointed by exactly one unacknowledged gratuitous ARP. Increment 1
   decouples them; BGP (increment 2) is what buys active-active (§3).
...
- [ ] **Floating-IP HA: separate attraction from delivery** — **[floating-ha.md](floating-ha.md)**. Today one node attracts (answers ARP), delivers (hosts the pod) and egresses, because all three are the same decision: the agent programs the `floating` map only on the target Port's node, and `floating_arp` answers only when the pod is `local_of` — *"programming the map is the advertisement"*. The cost is not ops-comfort but correctness: a live-migrating VM's public address is re-pointed by exactly **one** unacknowledged, never-repeated gratuitous ARP, and losing it black-holes the address for an ARP-cache lifetime — on the feature whose whole promise is a sub-second cutover with the address preserved
  - [ ] Increment 0 — a robust announcement: `AnnounceAddress` sends a spaced burst instead of one best-effort frame, and re-sends when a node newly wins an address (a latent migration bug on its own)
  - [ ] Increment 1 — decouple: every node programs `floating`; a new `float_announce` map (present only on the elected announcer) becomes the sole ARP/NDP gate; `from_uplink` gains the remote arm it never had (`remote_of(fe->net, fe->vpc_ip)` → `encap`); `from_overlay`'s VPC branch gains a floating probe **before** the `gateways` lookup (a public inner dst otherwise mis-delivers into the VPC's gateway pod); `to_pod`'s existing floating path DNATs it unchanged, and the reply is already DSR out the host's own uplink. Attraction becomes a rendezvous-hash election over `Ready` nodes that can serve the pool's link — no lease, no leader. Migration stops touching the external network at all
  - [ ] Increment 2 — BGP: several nodes advertise the same /32, the fabric ECMPs, BFD withdraws a dead one, and no shared L2 is needed. `ExternalPool.spec.advertisement` already reserves the `L2 | BGP` enum (presently dead — nothing reads it); the speaker belongs outside the datapath
