...
SRV='python3 -m http.server 8080 --bind ::'
...
EOF
$K -n "$NS" wait --for=condition=Ready pod/a1 pod/a2 pod/b1 pod/net0 --timeout=240s >/dev/null 2>&1 \
...
A1=$(vpcip a1); A2=$(vpcip a2); B1=$(vpcip b1)
A1F=$(fabricof a1)
echo "  a1: vpc=$A1 fabric=$A1F | a2: vpc=$A2 | b1(other VPC): vpc=$B1"
...
served a1 "http://$A2:8080/" && pass "a1 -> a2 across nodes (overlay delivery keyed on the VPC IP)" \
  || fail "intra-VPC cross-node failed"

# ---------------------------------------------------------------------------
phase "isolation + overlapping CIDRs"
# va and vb use the SAME CIDR: the same address must resolve within each VPC.
[ "$A1" = "$B1" ] || [ -n "$B1" ] && pass "overlapping VPC CIDRs coexist (b1=$B1 in vb, a1=$A1 in va)" \
  || fail "overlapping CIDR allocation broke"
refused a1 "http://$B1:8080/" && pass "cross-VPC traffic is dropped (unpeered, overlapping CIDRs)" \
...
served net0 "http://$A1F:8080/" && pass "net-0 pod -> VPC pod's FABRIC IP (bridge DNATs to the VPC IP)" \
  || fail "north-south via the fabric IP broke (the bridge is programmed from the claim now)"
...
EOF
# Overlapping CIDRs cannot be peered — the peering must not make them reachable.
sleep 8
refused a1 "http://$B1:8080/" && pass "peering does not defeat overlapping CIDRs (still unreachable)" \
  || fail "peered overlapping CIDRs became reachable (address collision)"
...
EOF
served a2 "http://$A1:8080/" && pass "the grouped client is admitted by the SG rules" \
  || fail "SG rules did not admit the grouped client"
